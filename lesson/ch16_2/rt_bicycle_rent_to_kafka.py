...
class RtBicycleRent(BaseStreamApp):
    ...
    def init_call(self, spark_session: SparkSession):
        ...
        spark_session.sql(f'''
            ...
            LOCATION 's3a://datalake-spark-sink-bennyjang/lesson/bicycle_rent_info'
    ...
    def sink_to_s3(self, last_stt_df: DataFrame, dttm: str, stream_df: DataFrame, epoch_id):
        ...
        if len(self.last_dttm) > 0:
            time_diff = datetime.strptime(dttm, '%Y-%m-%d %H:%M:%S') - datetime.strptime(self.last_dttm,
                                                                                         '%Y-%m-%d %H:%M:%S')
        ...
        if not cloned_last_stt_df.isEmpty():
            ...
            processed_df.coalesce(1).write \
                .format("parquet") \
                .mode('append') \
                .option("path", "s3a://datalake-spark-sink-bennyjang/lesson/bicycle_rent_info") \
                .partitionBy("ymd", "hh") \
                .save()
        ...
        if kafka_send_cnt > 0:
            ...
            self.logger.write_log('info', f'Kafka send Completed, Records: {kafka_send_cnt} / topic: {tgt_topic_nm})',
                                  epoch_id)
